...
"""
Profile LangGraph Agent (A2A over HTTP)
--------------------------------------
This service demonstrates an A2A‑style agent implemented with plain FastAPI (no Strands
runtime). It exposes HTTP endpoints compatible with the A2A data model and a discovery
agent card so other agents can find and communicate with it.

Why it’s fast
- No LLM planning loop is executed server‑side for this endpoint; `/a2a/messages` takes
  a message and synchronously returns structured output (either from your LangGraph flow
  if available, or a static stub). As a result, end‑to‑end latency is low and predictable.

Protocol alignment
- Discovery: `GET /.well-known/agent-card.json` returns metadata including
  `preferredTransport: HTTP`, which tells clients to use the HTTP path instead of JSON‑RPC.
- Messaging: `POST /a2a/messages` accepts an A2A Message and returns a Message with
  text and/or json parts. This is not JSON‑RPC; it’s a simple HTTP interaction for agents
  that support the HTTP transport.

Extension points
- If `agents.profile_agent_lg.handle_request_async` (or `handle_request`) exists in your
  environment, we call it to run your actual LangGraph graph. Otherwise we return a
  deterministic structured stub so consumers get a stable output during development.
"""
...
def extract_member_id(text: str) -> str:
    """Extract 6+ digit member id from free‑form text (robust parsing)."""
...
def agent_card():
    """A minimal agent card to enable discovery by A2A clients.

    Key fields
    - preferredTransport: HTTP → clients should call `/a2a/messages`.
    - skills: purely informative in this HTTP example; Strands/A2A SDKs will
      primarily look at the transport and URL when using this card.
    """
    return {
...
async def a2a_messages(msg: Message) -> Dict[str, Any]:
    """HTTP transport messaging endpoint.

    Accepts an A2A `Message` (no JSON‑RPC envelope) and returns an A2A `Message`
    with a text part (tool name) and a json part (structured payload). The
    orchestrator uses the `json` part for the final structured response.
    """
...
async def query_endpoint(body: Query) -> Dict[str, Any]:
    """Convenience endpoint for direct programmatic calls (non‑A2A)."""
